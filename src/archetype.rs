...
/// represents a single chunk in an archetype
struct ArchetypeChunk
{
    ...
    /// (cached) layout for every chunk allocations for this archetype
    ///
    /// this is copied from the parent `Archetype` to be used in its `Drop` implementation
    layout: Layout,
}
...
impl Archetype
{
    ...
    pub fn new(mut ty: Vec<TypeMeta>) -> Self
    {
        ...
        Self { meta, max, layout, chunks, free }
    }

    /// returns a slice of components via `TypeId`. see `Archetype::components`
    pub fn components_dyn(&self, ty: TypeId) -> &[u8]
    {
        let meta = self.meta
            .get(&ty)
    ...
    fn new_chunk(&mut self)
    {
        ...
        self.chunks.push(ArchetypeChunk { data, len, layout });
    }
}
...
impl Drop for ArchetypeChunk
{
    fn drop(&mut self)
    {
        unsafe
        {
            std::alloc::dealloc((*self.data.get()).as_ptr(), self.layout);
        }
    }
}
